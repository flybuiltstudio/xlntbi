...
- Nem lejáró linkek: a takarítás csak akkor zár le sort, ha `expires_at` nem null és elmúlt, vagy `download_count >= max_downloads`; a lejárat nélküli (`expires_at IS NULL`) vagy korlátlan (`max_downloads` 0/null) sorok kihagyva. A tároló-takarítás hivatkozás-gyűjtése ugyanígy aktívnak tekinti ezeket, tehát a fájljaik nem kerülnek a törölhető listára.
...
- Régóta fizetésre várók: `listOrders` eredményéből származtatott lista (`payment_status !== 'paid'` és `created_at < now() - 8 nap`), új blokk a `OrdersPanel` tetején + `payFilter` kiegészítése `stale` értékkel. Új adat nem kell hozzá.
- Havi statisztika-zárás: új `monthly_stats_snapshots` tábla (év, hónap, termék, megtekintés, rendelésszám, bevétel, `closed_at`) RLS-sel és `service_role` granttal; a Statisztika oldal a lezárt hónapoknál ebből olvas, a folyó hónapot élőben számolja.
