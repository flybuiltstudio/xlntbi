...
export const PRODUCT_SUMMARY_EN: Record<string, string> = {
  ...
  "telefonszamla-konyvelo":
    "Booking company phone bills with several numbers is the same tiring, error-prone manual work month after month. The Phone Bill Bookkeeper reads the PDF invoice, splits the items by phone number (by service on landline invoices), and with one click creates a file importable into your accounting software, with correct VAT rates and the private/business VAT split for phone services. It automatically recognises Magyar Telekom (both mobile and landline), Yettel and One business invoices, including older 2018–2022 Telekom layouts and scanned copies. Every split is reconciled against the invoice total, so items are only created when the amounts match to the last forint; exports are available for RLB, Kulcs-Soft, Novitax and Pénzszám, plus a generic CSV.",
